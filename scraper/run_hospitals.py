...
async def run_hospital_scraper():
    """Run hospital scraper only."""
    print("\n" + "=" * 60)
    print("Hope Platform - Hospital Scraper")
    print(f"Started at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 60)

    scraper = NYCHospitalsScraper()
    geocoder = Geocoder()
    ...
    with get_db() as db:
        ...
        for i, data in enumerate(raw_data, 1):
            ...
            try:
                ...
                # Add service types
                deduplicator.add_services(location, data.service_types)
    ...
    print("\n" + "=" * 60)
    print("Hospital Scraping Complete")
    print("=" * 60)
...
if __name__ == "__main__":
    asyncio.run(run_hospital_scraper())
