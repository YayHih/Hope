"""Run the detailed food pantries scraper."""
...
def run_food_pantries_scraper():
    """Run food pantries detailed scraper."""
    print("\n" + "=" * 60)
    print("Hope Platform - NYC Food Pantries Detailed Scraper")
    print(f"Started at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 60)

    # Initialize
    scraper = NYCFoodPantriesDetailed()
    geocoder = Geocoder()
        ...
        try:
            ...
            for idx, data in enumerate(raw_data, 1):
                ...
                try:
                    ...
                    if not data.street_address or data.street_address == 'Various locations':
                        print(f"    ⚠️  No specific address provided, skipping")
                        locations_skipped += 1
                        continue
